...
@Component
@ConditionalOnProperty(prefix = "findexso.bridge.printer", name = "enabled", havingValue = "true")
public class EscPosPrinterAdapter implements PrinterPort {
    ...
    public String installationHint() {
        ...
        return """
                ...
                findexso.bridge.printer.name con el nombre exacto.""";
    }
    ...
    static boolean matches(String actualName, String configuredName) {
        ...
        if (configuredName == null || configuredName.isBlank() || "default".equalsIgnoreCase(configuredName)) {
            return actual.contains(ThermalPrintConstants.XPRINTER_NAME)
                    || actual.contains(ThermalPrintConstants.XPRINTER_MODEL_PREFIX)
                    || actual.contains("pos");
        }
        String configured = configuredName.toLowerCase(Locale.ROOT);
        return actual.contains(configured)
                || (ThermalPrintConstants.XPRINTER_NAME.equals(configured)
                    && actual.contains(ThermalPrintConstants.XPRINTER_MODEL_PREFIX));
    }
