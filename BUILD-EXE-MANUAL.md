# Manual — Build `findexso-bridge.exe` (front + back integrados)
...
En resumen: **tú compilas con Java 21**; el cajero/usuario POS solo ejecuta `findexso-bridge.exe` tras instalar.
...
```powershell
cd "C:\Users\Oscar Jesus Sanabria\Documents\GitHub\Findexso-home-repo\9-findexso fix temas experienca\findexso-bridge\findexso-bridge-api\backend"
```
...
```
backend\dist\findexso-bridge\findexso-bridge.exe
```

Doble clic en ese `.exe` para arrancar el bridge (no es un asistente de instalación).

Instalador tipo setup (`findexso-bridge-1.0.0.exe`) — requiere **WiX 3+** en PATH:
...
```
backend\target\findexso-bridge-backend-1.0.0-SNAPSHOT.jar
```
...
java -jar target\findexso-bridge-backend-1.0.0-SNAPSHOT.jar
```
...
1. **File → Open** → carpeta `findexso-bridge-api/backend` (o el módulo Maven `findexso-bridge-backend`).
2. **File → Project Structure → Project** → SDK **Java 21** (`C:\Program Files\Java\jdk-21.0.10`).
3. Localice la clase **`com.findexso.bridge.FindexsoBridgeApplication`**.
4. Clic derecho → **Run 'FindexsoBridgeApplication'** (o Shift+F10).

**Run Configuration recomendada:**

| Campo | Valor |
|-------|--------|
| Main class | `com.findexso.bridge.FindexsoBridgeApplication` |
| Working directory | `...\findexso-bridge-api\backend` |
| JRE | 21 |
...
|--------------|----------------------|---------------------|--------------------------------------|-----------------------------------|
| **`win-exe`** | Carpeta `dist\findexso-bridge\` (copiar entera) | Doble clic → corre ya | No | No |
| **`win-installer`** | Un solo `dist\findexso-bridge-1.0.0.exe` | Asistente Siguiente → Instalar | **Sí** | **Sí** |
...
```
backend\dist\findexso-bridge-1.0.0.exe
```

Ese **único archivo** es el que copian los cajeros. No envíes la carpeta `findexso-bridge\` del perfil portable.

### Paso 3 — En cada PC del cajero

1. Doble clic en **`findexso-bridge-1.0.0.exe`**
2. **Siguiente** → carpeta (ej. `C:\Program Files\Findexso Bridge`) → **Siguiente**
3. Acceso directo en menú Inicio / escritorio (según opciones del asistente) → **Instalar** → **Finalizar**
4. Abrir **Findexso Bridge** desde Inicio
5. Navegador en **http://127.0.0.1:9095/** — impresora / lector QR

**Desinstalar:** Configuración → Aplicaciones → buscar **Findexso** / **findexso-bridge** → Desinstalar.

**No necesita Java** instalado en la PC del cajero.

**En otras PCs:** solo copia/ejecuta `findexso-bridge-1.0.0.exe`. **No** hace falta WiX ni JDK en el cajero (solo en tu PC al compilar).
...
```yaml
findexso.bridge.scale.enabled: true
findexso.bridge.scale.port-name: COM3   # puerto de la báscula en Windows
findexso.bridge.scale.mock-weight-kg: 0  # solo pruebas sin hardware: ej. 1.5
```

3. Front Findexso (`:4200`): formulario venta, producto con modo **WEIGHT**, no marcar “Peso manual”.
...
**Token por defecto (desarrollo):** `findexso-bridge-local-dev`
...
```javascript
localStorage.setItem('findexso.bridge.pairingToken', 'mi-token-secreto-largo');
```
...
Para uso diario en tu PC, usa solo **`win-exe`** (genera `dist\findexso-bridge\findexso-bridge.exe` portable, sin WiX).

### `Application destination directory ...\dist\findexso-bridge already exists`
...
```bash
rm -rf dist/findexso-bridge
mvn package -Pwin-exe
```

(Sin `-Pwith-ui` si ya corriste `npm run build` en `frontend/`.)

**Opción C — build completo** (el `pom.xml` ya borra `dist/findexso-bridge` antes de `jpackage`):
...
node ./node_modules/@angular/cli/bin/ng.js build findexso-bridge-ui --configuration production
...
**Rutas con espacios** (`Oscar Jesus Sanabria`, `9-findexso fix temas experienca`): si sigue fallando, prueba clonar/compilar en una ruta corta, por ejemplo `C:\dev\findexso-bridge`.
...
frontend/ (Angular)
       ...
       ▼
dist/findexso-bridge-ui/  ──copia──►  backend/target/classes/static/
       │
       │  mvn package
       ▼
findexso-bridge-backend-1.0.0-SNAPSHOT.jar
       │
       │  jpackage  (-Pwin-exe)
       ▼
target/dist/findexso-bridge-1.0.0.exe  ← entregar al usuario POS
```
